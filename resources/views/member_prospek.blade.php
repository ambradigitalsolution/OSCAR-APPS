    ...
    <div class="prospect-list">
        ...
        <div class="prospect-card">
            <div class="prospect-info">
                <div class="client-name">{{ $order->buyer_name }} ({{ $order->buyer_city }})</div>
                <div class="prospect-date">{{ $order->created_at->format('d F Y, H:i') }} - Nomor: {{ $order->order_id }}</div>
                <div class="prospect-notes">
                    Produk Utama: {{ $order->items->first()->product_name ?? 'Produk' }} 
                    (Total: {{ $order->items->sum('qty') }} Barang)
                </div>
            </div>
            <div class="status-container">
                ...
                <div class="timestamp-info">Update Terakhir: <br>{{ $order->updated_at->format('d M Y, H:i') }}</div>
            </div>
        </div>
        @empty
